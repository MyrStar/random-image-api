...
```bash
scp random-image-api-v1.0.5-src.tar.gz root@服务器IP:/tmp/
# 若服务器无法访问 npm，再额外上传离线依赖包（见第六节）
scp random-image-api-v1.0.5-node_modules-linux-x64.tar.gz root@服务器IP:/tmp/
```
...
tar -xzf /tmp/random-image-api-v1.0.5-src.tar.gz -C /opt/random-image-api --strip-components=1
...
tar -xzf /tmp/random-image-api-v1.0.5-node_modules-linux-x64.tar.gz \
    -C node_modules-offline --strip-components=2

docker build -f deploy/Dockerfile.offline -t random-image-api:v1.0.5 .
